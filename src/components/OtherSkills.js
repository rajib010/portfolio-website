import React from "react";
import { OtherSkillsInfo } from "@/config";
import { Card } from "@/components/ui/card";
import { FaStar, FaStarHalfAlt, FaRegStar } from "react-icons/fa";
const renderStars = (rating) => {
    const fullStars = Math.floor(rating);
    const halfStar = rating % 1 !== 0;
    const stars = [];
    // Full stars
    for (let i = 0; i < fullStars; i++) {
        stars.push(React.createElement(FaStar, { key: `full-${i}`, className: "text-gray-800" }));
    }
    // Half star
    if (halfStar) {
        stars.push(React.createElement(FaStarHalfAlt, { key: "half", className: "text-gray-800" }));
    }
    // Empty stars
    for (let i = stars.length; i < 5; i++) {
        stars.push(React.createElement(FaRegStar, { key: `empty-${i}`, className: "text-gray-400" }));
    }
    return React.createElement("div", { className: "flex justify-center space-x-1" }, stars);
};
const OtherSkills = () => {
    return (React.createElement("div", { className: "container my-5" },
        React.createElement("h1", { className: "text-center text-3xl font-bold mb-5" }, "Other Skills"),
        React.createElement("div", { className: "w-[90%] mx-auto grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4" }, OtherSkillsInfo.map((item) => (React.createElement(Card, { key: item.id, className: "relative rounded-lg p-4 text-center cursor-pointer shadow-xl transition hover:-translate-y-1" },
            React.createElement("div", { className: "center mb-2" },
                React.createElement("img", { src: item.pic, alt: "item-pic", className: "w-12 h-12" })),
            React.createElement("h2", { className: "font-semibold text-xl" }, item.title),
            React.createElement("div", { className: "m-1" }, renderStars(item.rating))))))));
};
export default OtherSkills;
