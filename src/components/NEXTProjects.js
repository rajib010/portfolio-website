...
const NEXTProjectsComponent = () => {
    ...
    return (React.createElement("div", { className: "container" },
        React.createElement("h1", { className: "text-left text-3xl font-bold my-5 ml-16" }, "NEXT Projects"),
        React.createElement("div", { className: "w-[90%] mx-auto grid grid-cols-3 md:grid-cols-4 gap-4" }, NEXTProjects.map((item) => {
            return (React.createElement(Card, { key: item.id, className: "rounded-lg p-4 text-center cursor-pointer shadow-xl transition hover:-translate-y-1" },
                ...
                React.createElement("h2", { className: "font-semibold text-xl" }, item.label),
                React.createElement("p", { className: "my-1 line-clamp-3" }, item.description),
                React.createElement("button", { className: "text-sm text-blue-600 underline mt-2 ml-2", onClick: () => openSheet(item) }, "View More Details")));
        })),
        showSheet && selectedProject && (React.createElement("div", { className: "fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-black bg-opacity-50" },
...
export default NEXTProjectsComponent;
