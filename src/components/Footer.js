    ...
    return (React.createElement("footer", { className: 'w-full bg-black text-white py-3 px-20' },
        React.createElement("div", { className: 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3' },
            React.createElement("div", { className: 'flex flex-col' },
                React.createElement("p", { className: 'font-bold mb-2 text-xl' }, "Get in touch"),
                FooterContact &&
                    FooterContact.map((item) => (React.createElement("p", { className: 'mb-4', key: item.id },
                        React.createElement(item.icon, { className: 'inline mr-4' }),
                        item.value)))),
            React.createElement("div", { className: 'flex flex-col' },
                React.createElement("p", { className: 'font-bold mb-2 text-xl' }, "Social Links"),
                FooterSocialLinks &&
                    FooterSocialLinks.map((item) => (React.createElement("p", { className: 'mb-4', key: item.id },
                        React.createElement(item.icon, { className: 'inline mr-4' }),
                        React.createElement(Link, { to: item.url, target: '_blank' }, item.label))))),
            React.createElement("div", { className: 'flex flex-col' },
                React.createElement("p", { className: 'font-bold mb-2 text-xl' }, "Send a message"),
                React.createElement("form", { onSubmit: handleSubmit },
                    React.createElement(Input, { className: 'text-black', value: message, onChange: (e) => setMessage(e.target.value) }),
                    React.createElement(Button, { type: 'submit', disabled: isSending, className: `max-w-[10vw] my-3 ${isSending ? 'bg-gray-400' : 'bg-green-700 hover:bg-green-900'}` }, isSending ? 'Sending...' : 'Send')))),
